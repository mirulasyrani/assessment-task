...
const Dashboard = () => {
  ...
  const logFrontendErrorToBackend = useCallback(async (error, context) => {
    try {
      ...
      console.warn('Sending frontend error:', errorDetails);
  ...
  return (
    <Layout>
      ...
      >
        ...
        <CandidateSummaryCards total={candidates.length} statusCounts={statusCounts} />
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      ...
                      logFrontendErrorToBackend(err, `status_update_failure_id_${c.id}`);
